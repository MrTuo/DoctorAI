...
class HeartDisease(models.Model):
    ...
    def __unicode__(self):
        return self.check.user.username
...
class ChronicKidneyDisease(models.Model):
    ...
    sg = models.BooleanField()# 比重（标称）：（1.005, 1.010, 1.015, 1.020, 1.025）选一个（注释：人体密度与水密度值比，选接近值）
    al = models.IntegerField()# 白蛋白（标称）：（0, 1, 2, 3, 4, 5）
    su = models.IntegerField()# 血糖（标称）：（0, 1, 2, 3, 4, 5）
    rbc = models.BooleanField()# 红细胞含量（标称）：（正常，异常）
    pc = models.BooleanField()# 脓细胞（标称）：（正常，异常）
    pcc = models.BooleanField()# 脓细胞团块（标称）：（可见，不可见）
    ...
    htn = models.BooleanField()# 高血压（标称）：（是，否）
    dm = models.BooleanField()# 糖尿病（标称）：（是否）
    cad = models.BooleanField()# 冠状动脉疾病（标称）：（是，否）
    appet = models.BooleanField()# 胃口(标称)：（好，差）
    pe = models.BooleanField()# 足部水肿（标称）：（是，否）
    ane = models.BooleanField()# 贫血症（标称）：（是，否）
